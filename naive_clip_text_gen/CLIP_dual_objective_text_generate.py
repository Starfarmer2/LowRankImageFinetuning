...
def find_matching_ids(embedding_vectors, query_vectors, topk=1):
    ...
    weight_table = embedding_vectors[torch.tensor(list(tokens_with_w.values()))]
    ...
    matching_ids = [list(tokens_with_w.values())[idx] for idx in indices.T[0]]
    return matching_ids
...
correct_text_inputs = tokenizer(["Five cars red hospital ten yuan"], padding=True, return_tensors="pt")
...
def fgsm_attack(original_token_vectors, original_position_vectors, correct_text_embeds, incorrect_text_embeds, encoder_layer, final_norm_layer, text_projection_layer, lr, L=60):
