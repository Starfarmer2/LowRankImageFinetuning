...
def fgsm_attack(original_token_vectors, original_position_vectors, correct_text_embeds, incorrect_text_embeds, encoder_layer, final_norm_layer, text_projection_layer, lr, L=0.8):#L=-0.68):
    ...
    scheduler = lr_scheduler.StepLR(optimizer, step_size=15, gamma=0.7)
    while loss > L:
        optimizer.zero_grad()
        ...
        loss = -cosine_similarity(incorrect_text_embeds, new_text_embeds)*2 + (diff_vec_from_incorrect @ diff_vec_from_incorrect.T)/100
        # print(f'loss: {loss}')
